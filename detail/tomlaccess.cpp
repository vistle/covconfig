...
toml::node *node_for_path(const Logger &logger, toml::table &root, const std::string &path, bool create)
{
    ...
    if (dot == std::string::npos) {
        ...
        if (bracket != std::string::npos) {
            ...
            if (!arr) {
                logger.error("-> node_for_path") << "expected array for section " << array << " in table " << root << std::endl;
                return nullptr;
            }
            auto idx = std::stoul(index);
            if (idx < 0) {
                logger.error("-> node_for_path") << "negative index " << idx << " for array " << array << " in table " << root << std::endl;
                return nullptr;
            }
            if (idx >= arr->size()) {
                if (!create) {
                    logger.error("-> node_for_path") << "index " << idx << " out of bounds for array " << array << " in table " << root
                              << std::endl;
                    return nullptr;
                }
        ...
        if (!result.second) {
            logger.error("-> node_for_path") << "could not insert section " << path << " into table " << root << std::endl;
            return nullptr;
        }
...
{
    logger.debug("-> table_for_section") << "looking for or creating section " << section << " in table " << root << std::endl;
    auto node = node_for_path(logger, root, section, create);
    if (!node) {
        logger.debug("-> table_for_section") << "looked for section " << section << " in table " << root << ", not found and not created" << std::endl;
        return nullptr;
    }
    if (auto tbl = node->as_table()) {
        logger.debug("-> table_for_section") << "looked for section " << section << " in table " << root << ", found" << *tbl << std::endl;
        return tbl;
    }
    logger.debug("-> table_for_section") << "looked for section " << section << " in table " << root << ", found not a table" << std::endl;
    return nullptr;
}
...
const toml::table *table_for_section(const Logger &logger, const toml::table &root, const std::string &section)
{
    ...
    if (!node) {
        logger.debug("-> table_for_section") << "looked for section " << section << " in table " << root << ", not found" << std::endl;
        return nullptr;
    }
    if (auto tbl = node->as_table()) {
        logger.debug("-> table_for_section") << "looked for section " << section << " in table " << root << ", found" << *tbl << std::endl;
                                                                    ...
                                                                    const std::string &name, bool optional)
{
    ...
    if (bracket != std::string::npos) {
        ...
        entry->debug("get_from_table") << "name " << name << " accesses an array: array=" << array << ", index=" << std::stoul(index) << std::endl;
        auto arr = (*tbl)[array];
        if (!arr) {
            (optional ? entry->debug("get_from_table") : entry->error("get_from_table")) << "node for array " << array << " not found" << std::endl;
            return std::optional<Type>();
        }
        if (!arr.is_array()) {
        ...
        auto idx = std::stoul(index);
        if (idx >= a->size()) {
            (optional ? entry->debug("get_from_table") : entry->error("get_from_table")) << "index " << idx << " out of bounds for array " << array << std::endl;
            return std::optional<Type>();
        }
...
const std::optional<typename Convert<Section>::Type> Convert<Section>::as(Entry *entry, size_t index, const toml::node &n)
{
